...
// ReviewsSection Component (keep as is)
const ReviewsSection = ({ productId, setReviewCount }) => {
  const [reviews, setReviews] = useState([]);
  const [loading, setLoading] = useState(true);
  const [averageRating, setAverageRating] = useState(0);
...
const Product = () => {
  ...
  useEffect(() => {
    if (isAutoPlaying && productData?.image?.length > 1) {
  ...
  const activeImage = productData.image[activeImageIndex];
  ...
  return (
    <div className="sm:pt-20 pt-5 max-w-6xl mx-auto px-4 md:px-8">
      <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          >
            ...
            {productData.image.length > 1 && (
              <div className="absolute top-4 right-4 z-10 bg-black/60 text-white px-3 py-1 rounded-full text-sm font-medium">
                {activeImageIndex + 1} / {productData.image.length}
              </div>
            )}
            ...
            <Zoom>
              <motion.img
                ...
                src={`${import.meta.env.VITE_BACKEND_URL}${activeImage.url}`}
                alt={`${productData.name} - Image ${activeImageIndex + 1}`}
                className="sm:w-[550px] w-[450px] h-auto rounded-none sm:rounded-lg cursor-zoom-in"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
        ...
        {/* Product Details Section (keep all existing code below this point) */}
        <div className="px-4">
          <h1 className="text-4xl philosopher-bold font-bold text-gray-900 mb-3">
            {productData.name}
          </h1>
          ...
          <div className="flex items-center gap-4 my-4">
            ...
            <p className="text-xl text-gray-500 line-through">
              {currency}
              {size ? size.actualprice : productData.sizes[0].actualprice}
            </p>
            <p className="top-2 left-2 bg-red-500 text-white text-sm font-bold px-3 py-1 rounded-full">
              {size ? size.offer : productData.sizes[0].offer}% OFF
            </p>
            ...
            <div className="flex gap-2 mt-2">
              {productData.sizes.map((item, index) => (
                <button
                  key={index}
                  onClick={() => handleSizeSelection(item.size)}
                  className={`border py-2 px-5 rounded-md transition-all ${
                    size?.size === item.size
                      ? "bg-black text-white"
                      : "bg-gray-100 hover:bg-gray-200"
                  }`}
          ...
          <div className="mt-6 w-[300px] flex gap-4">
            ...
            {/* Shopping Cart Sidebar (keep all existing code) */}
            <div
      ...
      {/* Tabs Section (keep all existing code) */}
      <div className="mt-16">
        <div className="flex border-b">
          <button
            onClick={() => setActiveTab("description")}
            className={`px-6 py-3 sm:text-lg text-base font-medium text-justify transition ${
              activeTab === "description"
                ? "border-b-2 border-black text-black"
                : "text-gray-500"
            }`}
